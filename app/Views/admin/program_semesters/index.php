...
<div class="content-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0"><i class="bi bi-diagram-3"></i> Program Semester Assignments</h4>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#assignmentModal" onclick="clearForm()">
            <i class="bi bi-plus-circle"></i> Assign Semester to Program
        </button>
    </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="assignmentModalLabel">Assign Semester to Program</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name" placeholder="e.g., Fall 2024, Spring 2025" required>
                            <div class="invalid-feedback"></div>
                        </div>
                        
                        <div class="col-md-6 mb-3">
                            <label for="code" class="form-label">Semester Code <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="code" name="code" placeholder="e.g., F2024, S2025" required>
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    assignmentsTable = $('#assignmentsTable').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: '<?= base_url('admin/program-semesters/getData') ?>',
            type: 'GET',
            dataType: 'json',
            dataSrc: 'data'
        },
...
function loadPrograms() {
    $.ajax({
        url: '<?= base_url('admin/program-semesters/getPrograms') ?>',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            ...
            response.data.forEach(function(prog) {
                select.append(`<option value="${prog.id}">${prog.name} (${prog.code}) - ${prog.department_name}</option>`);
            });
        },
        error: function() {
...
$('#assignmentForm').on('submit', function(e) {
    ...
    const url = isEditMode 
        ? `<?= base_url('admin/program-semesters/update') ?>/${$('#assignment_id').val()}`
        : '<?= base_url('admin/program-semesters/create') ?>';
...
function editAssignment(id) {
    ...
    $.ajax({
        url: `<?= base_url('admin/program-semesters/edit') ?>/${id}`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.success) {
        ...
        error: function(xhr) {
            showAlert('danger', 'Failed to load assignment data. Please try again.');
        }
    });
}
...
function deleteAssignment(id) {
    if (confirm('Are you sure you want to delete this assignment?')) {
        ...
        $.ajax({
            url: `<?= base_url('admin/program-semesters/delete') ?>/${id}`,
            type: 'POST',
            data: {
...
function clearForm() {
    ...
    $('#assignment_id').val('');
    $('#assignmentModalLabel').text('Assign Semester to Program');
    loadPrograms();
