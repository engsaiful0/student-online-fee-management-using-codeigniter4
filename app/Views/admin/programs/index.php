...
<div class="content-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0"><i class="bi bi-book"></i> Programs Management</h4>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#programModal" onclick="clearForm()">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control" id="duration_years" name="duration_years" value="4" min="1" max="10">
                        </div>
...
$(document).ready(function() {
    ...
    programsTable = $('#programsTable').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: '<?= base_url('admin/programs/getData') ?>',
            type: 'GET',
            dataType: 'json',
            dataSrc: 'data'
        },
        columns: [
            ...
            { data: 'code' },
            { data: 'name' },
            { data: 'duration_years', render: function(data) { return data + ' years'; } },
            { 
                data: 'status',
            ...
            { data: 'created_at' },
            {
                ...
                render: function(data, type, row) {
                    return `
                        <button class="btn btn-sm btn-warning" onclick="editProgram(${row.id})" title="Edit">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn btn-sm btn-danger" onclick="deleteProgram(${row.id})" title="Delete">
...
function loadDepartments() {
    $.ajax({
        url: '<?= base_url('admin/programs/getDepartments') ?>',
        type: 'GET',
        dataType: 'json',
...
$('#programForm').on('submit', function(e) {
    ...
    const url = isEditMode 
        ? `<?= base_url('admin/programs/update') ?>/${$('#program_id').val()}`
        : '<?= base_url('admin/programs/create') ?>';
...
function editProgram(id) {
    ...
    $.ajax({
        url: `<?= base_url('admin/programs/edit') ?>/${id}`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.success) {
                ...
                // Set department after a short delay to ensure dropdown is loaded
                setTimeout(function() {
                ...
                showAlert('danger', response.message || 'Failed to load program data');
            }
        },
        error: function(xhr) {
            showAlert('danger', 'Failed to load program data. Please try again.');
        }
    });
}
...
function deleteProgram(id) {
    if (confirm('Are you sure you want to delete this program?')) {
        ...
        const deleteBtn = $(`button[onclick="deleteProgram(${id})"]`);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showAlert('danger', response.message || 'Failed to delete program');
                }
            },
            error: function(xhr) {
